...
public class casino {
		...
		for (int i = 0; i < mapa.length; i++) {
			...
			for (int z = 0; z < mapa[i].length; z++) {
				...
				for (int k = 0; k < mapa[i][z].length; k++) {
					if(mapa[i][z][k] == "-1") {
						System.out.print(ANSI_GREEN + "" + mapa[i][z][k] + " " + ANSI_RESET);
					}else {
	...
	public static int juegocrash(double dinero) {
		...
		while (seguirsiono == true) {
			...
			do {
				eleccion = -1;
				try {
					System.out.println("Por cuanto quieres multiplicar tu apuesta? (x > 1)");
					eleccion = Double.parseDouble(sc.next());
				}catch(Exception e) {
					System.out.println("Caracter invalido");
				}

			} while (eleccion > 100.00 || eleccion < 1);
			
			sc.nextLine();
			el_ultimo_porcentaje = la_chicha(dados, eleccion);
	...
	public static double la_chicha(int dados, double numero) {
		...
		while (repetir == true) {
			...
			if (elnumerito >= 0.00 && elnumerito <= 1.00) {
				dados = rand.nextInt(30);
			} else if (elnumerito >= 1.01 && elnumerito <= 10.00) {
				dados = rand.nextInt(40);
			...
			} else if (elnumerito >= 70.01 && elnumerito <= 100.00) {
			...
			if(contador > 50) {
				contador = contador - (int)(elnumerito*20);
			}else {
				contador = 50;
			}
			...
			if(repetir == false) {
				if(numero >= elnumerito) {
					System.out.print(ANSI_RED + "x" + String.format("%.2f", elnumerito) + "\nCRASH " + ANSI_RESET);
				}else {
					System.out.print(ANSI_GREEN + "x" + String.format("%.2f", elnumerito) + "\nCRASH " + ANSI_RESET);
				}
